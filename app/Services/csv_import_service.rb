  ...
  def import
    ...
    CSV.foreach(@file.path, headers: true) do |row|
      ...
      if !Program.exists?(program_name: row['Programme Code'])
        ...
        notification = Notification.create!(identifier: row['Programme Code'],
                                            alert: 'New Programme added please update associated modules', isModule: false)
      ...
      if !Course.exists?(module_code: row['Module Code'])
        ...
        notification = Notification.create!(identifier: row['Module Code'],
                                            alert: 'New Module added please update accordingly', isModule: true)
      ...
      unless student.present?
        student = Student.create!(regID: row['Reg No.'], forename: row['Forename'],
                                  program_name: row['Programme Code'], surname: row['Surname'], program_id: program.id, status: row['Reg. Status'])
      end
      ...
      mark = Mark.create!(fst_grade: row['1st Grade'], scd_grade: row['2nd Grade'], course_id: course.id,
                          student_id: student.id)
      # classify marks accordingly
      if row['2nd Grade'].to_i < 50 && row['2nd Grade'].to_i != 0
        mark.status = 'F'
        mark.status = 'NA/F' if row['1st Grade'] == 'NA'
      elsif row['2nd Grade'].to_i >= 50
        mark.status = 'P'
        mark.status = 'NA/P' if row['1st Grade'] == 'NA'
      elsif row['1st Grade'] == 'NC'
        mark.status = 'NC'
        mark.fst_grade = 0
        mark.save
      ...
      if mark.final_score == mark.scd_grade && mark.final_score > 50
        mark.final_score = if mark.status == 'NA/P'
                             mark.scd_grade
                           else
                             50
                           end
    ...
    notification = Notification.create!(identifier: module_code, alert: "#{@count} students added to module",
                                        isModule: true)
    course = Course.find_by(module_code:)
    course.notifications << notification
    @count
  end

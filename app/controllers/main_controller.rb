...
class MainController < ApplicationController
  ...
  def home
    ...
    @course_list = []
    @course_list = params[:course_checkbox] if params[:course_checkbox].present?
  ...
  def check_students
    ...
    @people.each do |person|
      next unless person.updated_at.to_date < DateTime.current - 3.years

      unless @notifications.where(identifier: "Student Registration: #{person.regID}")
        Notification.create!(identifier: "Student Registration: #{person.regID}",
                             alert: 'Student data has not been updated for over 3 years.', isModule: false)
      end
    end
  end
end
